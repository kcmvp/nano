...
func IS() *Store {
	once.Do(func() {
		...
		db, err := buntdb.Open(filepath.Join(dir, "nano.db"))
		if err != nil {
...
func (store *Store) Registry(dbName string, prefix string) error {
	return store.impl.Update(func(tx *buntdb.Tx) error {
		...
		if _, err := tx.Get(key); !errors.Is(err, buntdb.ErrNotFound) {
			...
			if err == nil {
				return fmt.Errorf("db name '%s' already exists", dbName)
			}
		...
		if err := tx.AscendKeys(SchemaSpace(), func(_, value string) bool {
			if gjson.Get(value, "namespace").Str == prefix {
				exists = true
		...
		if exists {
			return fmt.Errorf("schema with namespace '%s' already exists", prefix)
		}
...
// If new fields are encountered in the payload, the schema is evolved to include
// new short keys for them, and this updated schema is persisted to the database.
// The function returns a `mo.Result[string]` containing the shortened JSON string
